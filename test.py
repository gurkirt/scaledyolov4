    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            if save_txt:
                ...
                # print(save_txt, out, paths[si])
                # txt_path = str(out / Path(paths[si]).stem)
                txt_path = paths[si].replace('images','preds')
                txt_path = txt_path.replace('.jpg','.txt')
                txt_dir = txt_path.split('/')
                txt_dir = '/'.join(txt_dir[:-1])
                
                # print(txt_path, txt_dir)
                if not os.path.isdir(txt_dir):
                    os.makedirs(txt_dir)
                fftid = open(txt_path, 'w')
                fftid.close()
                tpred = pred.clone()
                tpred[:, :4] = scale_coords(img[si].shape[1:], tpred[:, :4], shapes[si][0], shapes[si][1])  # to original
                for *xyxy, conf, cls in tpred:
                    if cls == 0:
                        ccc += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iou-thres', type=float, default=0.52, help='IOU threshold for NMS')
